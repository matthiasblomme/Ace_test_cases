...
| Component | Detail |
|---|---|
| **TEST_V13 / IS1** | ACE 13.0.7.0 integration node, admin `https://HomeServer:4418`. HTTP listener `7800` → flow at `http://localhost:7800/sap`. |
| **TEST_V12 / IS1** | ACE 12.0.12.17 integration node, admin `http://HomeServer:4416`. HTTP listener moved to `7802` (so both servers can bind at once) → `http://localhost:7802/sap`. |
